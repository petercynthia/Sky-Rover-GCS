...
namespace MissionPlanner.GCSViews
{
    public partial class ConfigParameters : Form
    {
        ...
        private void InitParams()
        {
            ...
            #region 飞行器结构
            Q_ENABLE.setup(1, 0, "Q_ENABLE", MainV2.comPort.MAV.param);
            Q_FRAME_CLASS.setup(ParameterMetaDataRepository.GetParameterOptionsInt("Q_FRAME_CLASS", MainV2.comPort.MAV.cs.firmware.ToString()).ToList(), "Q_FRAME_CLASS", MainV2.comPort.MAV.param);
            Q_FRAME_TYPE.setup(ParameterMetaDataRepository.GetParameterOptionsInt("Q_FRAME_TYPE", MainV2.comPort.MAV.cs.firmware.ToString()).ToList(), "Q_FRAME_TYPE", MainV2.comPort.MAV.param);
            #endregion

            #region 自稳Roll、PITCH、YAW
            Q_A_ANG_RLL_P.setup(0, 0, 1, "Q_A_ANG_RLL_P", MainV2.comPort.MAV.param);
            Q_A_ANG_PIT_P.setup(0, 0, 1,  "Q_A_ANG_PIT_P", MainV2.comPort.MAV.param);
            Q_A_ANG_YAW_P.setup(0, 0, 1, "Q_A_ANG_YAW_P", MainV2.comPort.MAV.param);
            #endregion
            ...
            Q_A_RAT_RLL_IMAX.setup(0, 0, 100, 0, "Q_A_RAT_RLL_IMAX", MainV2.comPort.MAV.param);
            Q_A_RAT_RLL_FF.setup(0, 0, 1, 0, "Q_A_RAT_RLL_FF", MainV2.comPort.MAV.param);
            Q_A_RAT_RLL_FILT.setup(0, 0, 1, 0, "Q_A_RAT_RLL_FILT", MainV2.comPort.MAV.param);
            #endregion
            ...
            Q_A_RAT_PIT_IMAX.setup(0, 0, 100, 0, "Q_A_RAT_PIT_IMAX", MainV2.comPort.MAV.param);
            Q_A_RAT_PIT_FF.setup(0, 0, 1, 0, "Q_A_RAT_PIT_FF", MainV2.comPort.MAV.param);
            Q_A_RAT_PIT_FILT.setup(0, 0, 1, 0, "Q_A_RAT_PIT_FILT", MainV2.comPort.MAV.param);
            ...
            Q_A_RAT_YAW_IMAX.setup(0, 0, 100, 0, "Q_A_RAT_YAW_IMAX", MainV2.comPort.MAV.param);
            Q_A_RAT_YAW_FF.setup(0, 0, 1, 0, "Q_A_RAT_YAW_FF", MainV2.comPort.MAV.param);
            Q_A_RAT_YAW_FILT.setup(0, 0, 1, 0, "Q_A_RAT_YAW_FILT", MainV2.comPort.MAV.param);
            #endregion



            #region 基本参数
            ARSPD_FBW_MIN_1.setup(0, 0, 1, 0, "ARSPD_FBW_MIN", MainV2.comPort.MAV.param);
            TRIM_ARSPD_CM.setup(0, 0, 100,  "TRIM_ARSPD_CM", MainV2.comPort.MAV.param);
            ARSPD_FBW_MAX.setup(0, 0, 1, 0, "ARSPD_FBW_MAX", MainV2.comPort.MAV.param);
            Q_ANGLE_MAX.setup(0, 0, 100, "Q_ANGLE_MAX", MainV2.comPort.MAV.param);
            Q_TRANSITION_MS.setup(0, 0, 1000,"Q_TRANSITION_MS", MainV2.comPort.MAV.param);
            Q_ASSIST_SPEED.setup(0, 0, 1, 0, "Q_ASSIST_SPEED", MainV2.comPort.MAV.param);
            Q_ASSIST_ANGLE.setup(0, 0, 1, 0, "Q_ASSIST_ANGLE", MainV2.comPort.MAV.param);
            Q_M_HOVER_LEARN.setup(1, 0, "Q_M_HOVER_LEARN", MainV2.comPort.MAV.param);
            #endregion

            #region 航点导航
            Q_GUIDED_MODE.setup(1, 0, "Q_GUIDED_MODE", MainV2.comPort.MAV.param);
            Q_RTL_MODE.setup(1, 0, "Q_RTL_MODE", MainV2.comPort.MAV.param);
            Q_RTL_ALT.setup(0, 0, 1, 0, "Q_RTL_ALT", MainV2.comPort.MAV.param);
            Q_WP_SPEED.setup(0, 0, 100, 0, "Q_WP_SPEED", MainV2.comPort.MAV.param);
            Q_WP_RADIUS.setup(0, 0, 100, 0, "Q_WP_RADIUS", MainV2.comPort.MAV.param);
            Q_WP_SPEED_UP.setup(0, 0, 100, 0, "Q_WP_SPEED_UP", MainV2.comPort.MAV.param);
            Q_WP_SPEED_DN.setup(0, 0, 100, 0, "Q_WP_SPEED_DN", MainV2.comPort.MAV.param);
            Q_LAND_SPEED.setup(0, 0, 100, 0, "Q_LAND_SPEED", MainV2.comPort.MAV.param);
            Q_LAND_FINAL_ALT.setup(0, 0, 1, 0, "Q_LAND_FINAL_ALT", MainV2.comPort.MAV.param);
            #endregion


            #region 倾转垂起功能
            //确定控制哪几个倾转电机项
            setQ_TILT_MASK_Value((int)MainV2.comPort.MAV.param["Q_TILT_MASK"].Value);

            Q_TILT_TYPE.setup(
            ParameterMetaDataRepository.GetParameterOptionsInt("Q_TILT_TYPE", MainV2.comPort.MAV.cs.firmware.ToString())
                .ToList(), "Q_TILT_TYPE", MainV2.comPort.MAV.param);
            Q_TILT_RATE_UP.setup(0, 0, 1, 0, "Q_TILT_RATE_UP", MainV2.comPort.MAV.param);
            Q_TILT_RATE_DN.setup(0, 0, 1, 0, "Q_TILT_RATE_DN", MainV2.comPort.MAV.param);
            Q_TILT_MAX.setup(0, 0, 1, 0, "Q_TILT_MAX", MainV2.comPort.MAV.param);
            Q_TILT_YAW_ANGLE.setup(0, 0, 1,"Q_TILT_YAW_ANGLE", MainV2.comPort.MAV.param);
            #endregion

            #region 尾座式垂起功能
            Q_TAILSIT_ANGLE.setup(0, 0, 1, "Q_TAILSIT_ANGLE", MainV2.comPort.MAV.param);
            Q_TAILSIT_INPUT.setup(
            ParameterMetaDataRepository.GetParameterOptionsInt("Q_TAILSIT_INPUT", MainV2.comPort.MAV.cs.firmware.ToString())
               .ToList(), "Q_TAILSIT_INPUT", MainV2.comPort.MAV.param);
            //手动控制舵面
            Q_TAILSIT_MASK.setup("Q_TAILSIT_MASK", MainV2.comPort.MAV.param);
            Q_TAILSIT_MASK.myLabel1.Text = "手动控制舵面";
            Q_TAILSIT_MASK.Controls.Remove(Q_TAILSIT_MASK.label1);
            Q_TAILSIT_MASKCH.setup(
            ParameterMetaDataRepository.GetParameterOptionsInt("Q_TAILSIT_MASKCH", MainV2.comPort.MAV.cs.firmware.ToString())
               .ToList(), "Q_TAILSIT_MASKCH", MainV2.comPort.MAV.param);
            #endregion
        }
        /// <summary>
        /// 
        ...
        private void SetQ_ParametersEnable(bool enable)
        {
            ...
            Q_FRAME_CLASS.Enabled = enable;//(ParameterMetaDataRepository.GetParameterOptionsInt("Q_FRAME_CLASS", MainV2.comPort.MAV.cs.firmware.ToString()).ToList(), "Q_FRAME_CLASS", MainV2.comPort.MAV.param);
            Q_FRAME_TYPE.Enabled = enable;//(ParameterMetaDataRepository.GetParameterOptionsInt("Q_FRAME_TYPE", MainV2.comPort.MAV.cs.firmware.ToString()).ToList(), "Q_FRAME_TYPE", MainV2.comPort.MAV.param);
            #endregion

            #region 自稳Roll、PITCH、YAW
            Q_A_RAT_RLL_P.Enabled = enable;//(0, 0, 1, 0, "Q_A_ANG_RLL_P", MainV2.comPort.MAV.param);
            Q_A_ANG_PIT_P.Enabled = enable;//(0, 0, 1, 0, "Q_A_ANG_PIT_P", MainV2.comPort.MAV.param);
            Q_A_ANG_YAW_P.Enabled = enable;//(0, 0, 1, 0, "Q_A_ANG_YAW_P", MainV2.comPort.MAV.param);
            #endregion

            #region  Roll 速率
            Q_A_ANG_RLL_P.Enabled = enable;//(0, 0, 1, 0, "Q_A_RAT_RLL_P", MainV2.comPort.MAV.param);
            Q_A_RAT_RLL_I.Enabled = enable;//(0, 0, 100, 0, "Q_A_RAT_RLL_I", MainV2.comPort.MAV.param);
            Q_A_RAT_RLL_D.Enabled = enable;//(0, 0, 100, 0, "Q_A_RAT_RLL_D", MainV2.comPort.MAV.param);
            Q_A_RAT_RLL_IMAX.Enabled = enable;//(0, 0, 100, 0, "Q_A_RAT_RLL_IMAX", MainV2.comPort.MAV.param);
            Q_A_RAT_RLL_FF.Enabled = enable;//(0, 0, 100, 0, "Q_A_RAT_RLL_FF", MainV2.comPort.MAV.param);
            Q_A_RAT_RLL_FILT.Enabled = enable;//(0, 0, 1, 0, "Q_A_RAT_RLL_FILT", MainV2.comPort.MAV.param);
            #endregion

            #region PITCH速率
            Q_A_RAT_PIT_P.Enabled = enable;//(0, 0, 1, 0, "Q_A_RAT_PIT_P", MainV2.comPort.MAV.param);
            Q_A_RAT_PIT_I.Enabled = enable;//(0, 0, 100, 0, "Q_A_RAT_PIT_I", MainV2.comPort.MAV.param);
            Q_A_RAT_PIT_D.Enabled = enable;//(0, 0, 1, 0, "Q_A_RAT_PIT_D", MainV2.comPort.MAV.param);
            Q_A_RAT_PIT_IMAX.Enabled = enable;//(0, 0, 1, 0, "Q_A_RAT_PIT_IMAX", MainV2.comPort.MAV.param);
            Q_A_RAT_PIT_FF.Enabled = enable;//(0, 0, 1, 0, "Q_A_RAT_PIT_FF", MainV2.comPort.MAV.param);
            Q_A_RAT_PIT_FILT.Enabled = enable;//(0, 0, 100, 0, "Q_A_RAT_PIT_FILT", MainV2.comPort.MAV.param);

            #endregion

            #region YAW 速率
            Q_A_RAT_YAW_P.Enabled = enable;//(0, 0, 1, 0, "Q_A_RAT_YAW_P", MainV2.comPort.MAV.param);
            Q_A_RAT_YAW_I.Enabled = enable;//(0, 0, 1, 0, "Q_A_RAT_YAW_I", MainV2.comPort.MAV.param);
            Q_A_RAT_YAW_D.Enabled = enable;//(0, 0, 1, 0, "Q_A_RAT_YAW_D", MainV2.comPort.MAV.param);
            Q_A_RAT_YAW_IMAX.Enabled = enable;//(0, 0, 100, 0, "Q_A_RAT_YAW_IMAX", MainV2.comPort.MAV.param);
            Q_A_RAT_YAW_FF.Enabled = enable;//(0, 0, 1, 0, "Q_A_RAT_YAW_FF", MainV2.comPort.MAV.param);
            Q_A_RAT_YAW_FILT.Enabled = enable;//(0, 0, 1, 0, "Q_A_RAT_YAW_FILT", MainV2.comPort.MAV.param);
            #endregion

            #region 旋翼/固定翼模式转换
            Q_ANGLE_MAX.Enabled = enable;
            Q_TRANSITION_MS.Enabled = enable;//(0, 0, 100, 0, "Q_TRANSITION_MS", MainV2.comPort.MAV.param);
            Q_ASSIST_SPEED.Enabled = enable;//(0, 0, 1, 0, "Q_ASSIST_SPEED", MainV2.comPort.MAV.param);
            Q_ASSIST_ANGLE.Enabled = enable;//(0, 0, 1, 0, "Q_ASSIST_ANGLE", MainV2.comPort.MAV.param);
            Q_M_HOVER_LEARN.Enabled = enable;//(1, 0, "Q_M_HOVER_LEARN", MainV2.comPort.MAV.param);
            #endregion

            #region 航点导航
            Q_GUIDED_MODE.Enabled = enable;//(1, 0, "Q_GUIDED_MODE", MainV2.comPort.MAV.param);
            Q_RTL_MODE.Enabled = enable;//(1, 0, "Q_RTL_MODE", MainV2.comPort.MAV.param);
            Q_RTL_ALT.Enabled = enable;//(0, 0, 1, 0, "Q_RTL_ALT", MainV2.comPort.MAV.param);
            Q_WP_SPEED.Enabled = enable;//(0, 0, 1, 0, "Q_WP_SPEED", MainV2.comPort.MAV.param);
            Q_WP_RADIUS.Enabled = enable;//(0, 0, 100, 0, "Q_WP_RADIUS", MainV2.comPort.MAV.param);
            Q_WP_SPEED_UP.Enabled = enable;//(0, 0, 1, 0, "Q_WP_SPEED_UP", MainV2.comPort.MAV.param);
            Q_WP_SPEED_DN.Enabled = enable;//(0, 0, 1, 0, "Q_WP_SPEED_DN", MainV2.comPort.MAV.param);
            Q_LAND_SPEED.Enabled = enable;//(0, 0, 1, 0, "Q_LAND_SPEED", MainV2.comPort.MAV.param);
            Q_LAND_FINAL_ALT.Enabled = enable;//(0, 0, 1, 0, "Q_LAND_FINAL_ALT", MainV2.comPort.MAV.param);
            #endregion
            ...
            Q_TILT_TYPE.Enabled = enable;//(ParameterMetaDataRepository.GetParameterOptionsInt("Q_TILT_TYPE", MainV2.comPort.MAV.cs.firmware.ToString()).ToList(), "Q_TILT_TYPE", MainV2.comPort.MAV.param);
            Q_TILT_RATE_UP.Enabled = enable;//(0, 0, 1, 0, "Q_TILT_RATE_UP", MainV2.comPort.MAV.param);
            Q_TILT_RATE_DN.Enabled = enable;//(0, 0, 1, 0, "Q_TILT_RATE_DN", MainV2.comPort.MAV.param);
            Q_TILT_MAX.Enabled = enable;//(0, 0, 1, 0, "Q_TILT_MAX", MainV2.comPort.MAV.param);
            Q_TILT_YAW_ANGLE.Enabled = enable;//(0, 0, 1, 0, "Q_TILT_YAW_ANGLE", MainV2.comPort.MAV.param);
            #endregion

            #region 尾座式垂起功能
            Q_TAILSIT_ANGLE.Enabled = enable;//(0, 0, 1, 0, "Q_TAILSIT_ANGLE", MainV2.comPort.MAV.param);
            Q_TAILSIT_INPUT.Enabled = enable;//(ParameterMetaDataRepository.GetParameterOptionsInt("Q_TAILSIT_INPUT", MainV2.comPort.MAV.cs.firmware.ToString()).ToList(), "Q_TAILSIT_INPUT", MainV2.comPort.MAV.param);
            Q_TAILSIT_MASK.Enabled = enable;//(0, 0, 1, 0, "Q_TAILSIT_MASK", MainV2.comPort.MAV.param);

            Q_TAILSIT_MASKCH.Enabled = enable;//(ParameterMetaDataRepository.GetParameterOptionsInt("Q_TAILSIT_MASKCH", MainV2.comPort.MAV.cs.firmware.ToString()).ToList(), "Q_TAILSIT_MASKCH", MainV2.comPort.MAV.param);
            #endregion
        ...
        void EEPROM_View_float_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                } else if (sender.GetType() == typeof(MAVLinkTextBox)) {
        ...
        private void BUT_writePIDS_Click(object sender, EventArgs e)
        {
            ...
            foreach (string value in temp.Keys)
            {
                try
                {
                    ...
                    if (value == "Q_TILT_MASK") MainV2.comPort.setParam(value, (int)changes[value]);
                    else MainV2.comPort.setParam(value, (float)changes[value]);
