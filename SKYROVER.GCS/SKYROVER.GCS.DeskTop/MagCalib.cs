...
using SKYROVER.GCS.DeskTop.HIL;
    ...
    public class MagCalib
    {
        ...
        static bool ReceviedPacket(MAVLink.MAVLinkMessage rawpacket)
        {
            ...
            else if (rawpacket.msgid == (byte) MAVLink.MAVLINK_MSG_ID.RAW_IMU)
            {
                ...
                // values
                float rawmx = packet.xmag - (float) MainUI.comPort.MAV.cs.mag_ofs_x;
                float rawmy = packet.ymag - (float) MainUI.comPort.MAV.cs.mag_ofs_y;
                float rawmz = packet.zmag - (float) MainUI.comPort.MAV.cs.mag_ofs_z;
        ...
        static void prd_DoWork(IProgressReporterDialogue sender)
        {
            ...
            // turn learning off
            MainUI.comPort.setParam("COMPASS_LEARN", 0);

            bool havecompass2 = false;
            bool havecompass3 = false;

            if (MainUI.comPort.MAV.param.ContainsKey("COMPASS_OFS_X"))
            {
                MainUI.comPort.setParam("COMPASS_OFS_X", 0, true);
                MainUI.comPort.setParam("COMPASS_OFS_Y", 0, true);
                MainUI.comPort.setParam("COMPASS_OFS_Z", 0, true);

                MainUI.comPort.setParam("COMPASS_DIA_X", 1, true);
                MainUI.comPort.setParam("COMPASS_DIA_Y", 1, true);
                MainUI.comPort.setParam("COMPASS_DIA_Z", 1, true);

                MainUI.comPort.setParam("COMPASS_ODI_X", 0, true);
                MainUI.comPort.setParam("COMPASS_ODI_Y", 0, true);
                MainUI.comPort.setParam("COMPASS_ODI_Z", 0, true);
            }

            //compass2 get mag2 offsets
            if (MainUI.comPort.MAV.param.ContainsKey("COMPASS_OFS2_X"))
            {
                MainUI.comPort.setParam("COMPASS_OFS2_X", 0, true);
                MainUI.comPort.setParam("COMPASS_OFS2_Y", 0, true);
                MainUI.comPort.setParam("COMPASS_OFS2_Z", 0, true);

                MainUI.comPort.setParam("COMPASS_DIA2_X", 1, true);
                MainUI.comPort.setParam("COMPASS_DIA2_Y", 1, true);
                MainUI.comPort.setParam("COMPASS_DIA2_Z", 1, true);

                MainUI.comPort.setParam("COMPASS_ODI2_X", 0, true);
                MainUI.comPort.setParam("COMPASS_ODI2_Y", 0, true);
                MainUI.comPort.setParam("COMPASS_ODI2_Z", 0, true);

                havecompass2 = true;
            }

            //compass3
            if (MainUI.comPort.MAV.param.ContainsKey("COMPASS_OFS3_X"))
            {
                MainUI.comPort.setParam("COMPASS_OFS3_X", 0, true);
                MainUI.comPort.setParam("COMPASS_OFS3_Y", 0, true);
                MainUI.comPort.setParam("COMPASS_OFS3_Z", 0, true);

                MainUI.comPort.setParam("COMPASS_DIA3_X", 1, true);
                MainUI.comPort.setParam("COMPASS_DIA3_Y", 1, true);
                MainUI.comPort.setParam("COMPASS_DIA3_Z", 1, true);

                MainUI.comPort.setParam("COMPASS_ODI3_X", 0, true);
                MainUI.comPort.setParam("COMPASS_ODI3_Y", 0, true);
                MainUI.comPort.setParam("COMPASS_ODI3_Z", 0, true);
            ...
            byte backupratesens = MainUI.comPort.MAV.cs.ratesensors;

            byte backuprateatt = MainUI.comPort.MAV.cs.rateattitude;
            byte backupratepos = MainUI.comPort.MAV.cs.rateposition;

            MainUI.comPort.MAV.cs.ratesensors = 2;
            MainUI.comPort.MAV.cs.rateattitude = 0;
            MainUI.comPort.MAV.cs.rateposition = 0;

            MainUI.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.ALL, 0);
            MainUI.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.RAW_SENSORS, 50);

            // subscribe to data packets
            var sub = MainUI.comPort.SubscribeToPacketType(MAVLink.MAVLINK_MSG_ID.RAW_IMU, ReceviedPacket);

            var sub2 = MainUI.comPort.SubscribeToPacketType(MAVLink.MAVLINK_MSG_ID.SCALED_IMU2, ReceviedPacket);

            var sub3 = MainUI.comPort.SubscribeToPacketType(MAVLink.MAVLINK_MSG_ID.SCALED_IMU3, ReceviedPacket);
            ...
            MainUI.comPort.UnSubscribeToPacketType(sub);
            MainUI.comPort.UnSubscribeToPacketType(sub2);
            MainUI.comPort.UnSubscribeToPacketType(sub3);

            // restore old sensor rate
            MainUI.comPort.MAV.cs.ratesensors = backupratesens;
            MainUI.comPort.MAV.cs.rateattitude = backuprateatt;
            MainUI.comPort.MAV.cs.rateposition = backupratepos;
            MainUI.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.RAW_SENSORS, MainUI.comPort.MAV.cs.ratesensors);

            MainUI.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.POSITION, MainUI.comPort.MAV.cs.rateposition);
                // request gps
            MainUI.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.EXTRA1, MainUI.comPort.MAV.cs.rateattitude);
                // request attitude
            MainUI.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.EXTRA2, MainUI.comPort.MAV.cs.rateattitude);
                // request vfr
            MainUI.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.EXTRA3, MainUI.comPort.MAV.cs.ratesensors);
                // request extra stuff - tridge
            MainUI.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.RAW_SENSORS, MainUI.comPort.MAV.cs.ratesensors);
                // request raw sensor
            MainUI.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.RC_CHANNELS, MainUI.comPort.MAV.cs.raterc);
                // request rc info

            if (MainUI.speechEnable)
            {
                MainUI.speechEngine.SpeakAsync("Compass Calibration Complete");
            }
            else
            {
                Console.Beep();
            ...
            if (MainUI.comPort.MAV.param.ContainsKey("COMPASS_DIA_X"))
            {
                ellipsoid = true;
            }
        ...
        {
            if (MainUI.comPort.MAV.param.ContainsKey("COMPASS_OFS_X") && MainUI.comPort.BaseStream.IsOpen)
            {
                try
                {
                    // disable learning
                    ...
                    if (
                        !MainUI.comPort.SetSensorOffsets(MAVLinkInterface.sensoroffsetsenum.magnetometer, (float) ofs[0],
                            (float) ofs[1], (float) ofs[2]))
                    {
                        // set values
                        MainUI.comPort.setParam("COMPASS_OFS_X", (float) ofs[0]);
                        MainUI.comPort.setParam("COMPASS_OFS_Y", (float) ofs[1]);
                        MainUI.comPort.setParam("COMPASS_OFS_Z", (float) ofs[2]);
                    }
                    else
                    {
                        // Need to reload these params into the param list if SetSensorOffsets() was used
                        MainUI.comPort.GetParam("COMPASS_OFS_X");
                        MainUI.comPort.GetParam("COMPASS_OFS_Y");
                        MainUI.comPort.GetParam("COMPASS_OFS_Z");
                    }

                    if (ofs.Length > 5 && MainUI.comPort.MAV.param.ContainsKey("COMPASS_DIA_X"))
                    {
                        // ellipsoid
                        MainUI.comPort.setParam("COMPASS_DIA_X", (float)ofs[3]);
                        MainUI.comPort.setParam("COMPASS_DIA_Y", (float)ofs[4]);
                        MainUI.comPort.setParam("COMPASS_DIA_Z", (float)ofs[5]);

                        MainUI.comPort.setParam("COMPASS_ODI_X", (float)ofs[6]);
                        MainUI.comPort.setParam("COMPASS_ODI_Y", (float)ofs[7]);
                        MainUI.comPort.setParam("COMPASS_ODI_Z", (float)ofs[8]);
                    }
                }
                catch
                {
        ...
        {
            if (MainUI.comPort.MAV.param.ContainsKey("COMPASS_OFS2_X") && MainUI.comPort.BaseStream.IsOpen)
            {
                try
                {
                    // disable learning
                    ...
                    if (
                        !MainUI.comPort.SetSensorOffsets(MAVLinkInterface.sensoroffsetsenum.second_magnetometer,
                            (float) ofs[0], (float) ofs[1], (float) ofs[2]))
                    {
                        // set values
                        MainUI.comPort.setParam("COMPASS_OFS2_X", (float) ofs[0]);
                        MainUI.comPort.setParam("COMPASS_OFS2_Y", (float) ofs[1]);
                        MainUI.comPort.setParam("COMPASS_OFS2_Z", (float) ofs[2]);
                    }
                    else
                    {
                        // Need to reload these params into the param list if SetSensorOffsets() was used
                        MainUI.comPort.GetParam("COMPASS_OFS2_X");
                        MainUI.comPort.GetParam("COMPASS_OFS2_Y");
                        MainUI.comPort.GetParam("COMPASS_OFS2_Z");
                    }
                    if (ofs.Length > 5 && MainUI.comPort.MAV.param.ContainsKey("COMPASS_DIA2_X"))
                    {
                        // ellipsoid
                        MainUI.comPort.setParam("COMPASS_DIA2_X", (float)ofs[3]);
                        MainUI.comPort.setParam("COMPASS_DIA2_Y", (float)ofs[4]);
                        MainUI.comPort.setParam("COMPASS_DIA2_Z", (float)ofs[5]);

                        MainUI.comPort.setParam("COMPASS_ODI2_X", (float)ofs[6]);
                        MainUI.comPort.setParam("COMPASS_ODI2_Y", (float)ofs[7]);
                        MainUI.comPort.setParam("COMPASS_ODI2_Z", (float)ofs[8]);
                    }
                }
                catch
                {
        ...
        {
            if (MainUI.comPort.MAV.param.ContainsKey("COMPASS_OFS3_X") && MainUI.comPort.BaseStream.IsOpen)
            {
                try
                {
                    // disable learning
                    MainUI.comPort.setParam("COMPASS_LEARN", 0);
                    {
                        MainUI.comPort.setParam("COMPASS_OFS3_X", (float)ofs[0]);
                        MainUI.comPort.setParam("COMPASS_OFS3_Y", (float)ofs[1]);
                        MainUI.comPort.setParam("COMPASS_OFS3_Z", (float)ofs[2]);
                    }
                    if (ofs.Length > 5 && MainUI.comPort.MAV.param.ContainsKey("COMPASS_DIA3_X"))
                    {
                        // ellipsoid
                        MainUI.comPort.setParam("COMPASS_DIA3_X", (float)ofs[3]);
                        MainUI.comPort.setParam("COMPASS_DIA3_Y", (float)ofs[4]);
                        MainUI.comPort.setParam("COMPASS_DIA3_Z", (float)ofs[5]);

                        MainUI.comPort.setParam("COMPASS_ODI3_X", (float)ofs[6]);
                        MainUI.comPort.setParam("COMPASS_ODI3_Y", (float)ofs[7]);
                        MainUI.comPort.setParam("COMPASS_ODI3_Z", (float)ofs[8]);
                    }
                }
                catch
                {
