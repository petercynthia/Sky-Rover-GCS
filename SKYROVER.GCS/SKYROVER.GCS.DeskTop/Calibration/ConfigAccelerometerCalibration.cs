    ...
    public partial class ConfigAccelerometerCalibration : UserControl, IActivate, IDeactivate
    {
        ...
        private void BUT_calib_accell_Click(object sender, EventArgs e)
        {
            if (_incalibrate)
            {
                ...
                try
                {
                    MainUI.comPort.sendPacket(new MAVLink.mavlink_command_ack_t {command = 1, result = count},
                        MainUI.comPort.sysidcurrent, MainUI.comPort.compidcurrent);
                }
                catch
                {
            ...
            try
            {
                ...
                MainUI.comPort.doCommand(MAVLink.MAV_CMD.PREFLIGHT_CALIBRATION, 0, 0, 0, 0, 1, 0, 0);

                _incalibrate = true;

                MainUI.comPort.SubscribeToPacketType(MAVLink.MAVLINK_MSG_ID.STATUSTEXT, receivedPacket);
            ...
            if (arg.msgid == (uint)MAVLink.MAVLINK_MSG_ID.STATUSTEXT)
            {
                 ...
                 message.ToLower().Contains("calibration failed"))
                {
                    try
                    {
                        ...
                        _incalibrate = false;
                        MainUI.comPort.UnSubscribeToPacketType(MAVLink.MAVLINK_MSG_ID.STATUSTEXT, receivedPacket);
                    }
                    catch
                    {
                    }
        ...
        private void BUT_level_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                MainUI.comPort.doCommand(MAVLink.MAV_CMD.PREFLIGHT_CALIBRATION, 0, 0, 0, 0, 2, 0, 0);
