...
namespace SKYROVER.GCS.DeskTop.Joystick
{
    public class Joystick : IDisposable
    {
        ...
        public Joystick()
        {
            ...
            {
                loadconfig("joystickbuttons" + MainUI.comPort.MAV.cs.firmware + ".xml",
                    "joystickaxis" + MainUI.comPort.MAV.cs.firmware + ".xml");
            }
            else if (MainUI.comPort.MAV.cs.firmware == Firmwares.ArduCopter2)
            {
                loadconfig("joystickbuttons" + MainUI.comPort.MAV.cs.firmware + ".xml",
                    "joystickaxis" + MainUI.comPort.MAV.cs.firmware + ".xml");
            }
        ...
        void mainloop()
        {
            while (enabled && joystick != null && !joystick.IsDisposed)
            {
                try
                {
                    ...
                    {
                        if (getJoystickAxis(1) != Joystick.joystickaxis.None)
                            MainUI.comPort.MAV.cs.rcoverridech1 = pickchannel(1, JoyChannels[1].axis,
                                JoyChannels[1].reverse, JoyChannels[1].expo);
                                //(ushort)(((int)state.Rz / 65.535) + 1000);
                        if (getJoystickAxis(2) != Joystick.joystickaxis.None)
                            MainUI.comPort.MAV.cs.rcoverridech2 = pickchannel(2, JoyChannels[2].axis,
                                JoyChannels[2].reverse, JoyChannels[2].expo);
                                //(ushort)(((int)state.Y / 65.535) + 1000);
                    }
                    if (getJoystickAxis(3) != Joystick.joystickaxis.None)
                        MainUI.comPort.MAV.cs.rcoverridech3 = pickchannel(3, JoyChannels[3].axis, JoyChannels[3].reverse,
                            JoyChannels[3].expo); //(ushort)(1000 - ((int)slider[0] / 65.535) + 1000);
                    if (getJoystickAxis(4) != Joystick.joystickaxis.None)
                        MainUI.comPort.MAV.cs.rcoverridech4 = pickchannel(4, JoyChannels[4].axis, JoyChannels[4].reverse,
                            JoyChannels[4].expo); //(ushort)(((int)state.X / 65.535) + 1000);

                    if (getJoystickAxis(5) != Joystick.joystickaxis.None)
                        MainUI.comPort.MAV.cs.rcoverridech5 = pickchannel(5, JoyChannels[5].axis, JoyChannels[5].reverse,
                            JoyChannels[5].expo);
                    if (getJoystickAxis(6) != Joystick.joystickaxis.None)
                        MainUI.comPort.MAV.cs.rcoverridech6 = pickchannel(6, JoyChannels[6].axis, JoyChannels[6].reverse,
                            JoyChannels[6].expo);
                    if (getJoystickAxis(7) != Joystick.joystickaxis.None)
                        MainUI.comPort.MAV.cs.rcoverridech7 = pickchannel(7, JoyChannels[7].axis, JoyChannels[7].reverse,
                            JoyChannels[7].expo);
                    if (getJoystickAxis(8) != Joystick.joystickaxis.None)
                        MainUI.comPort.MAV.cs.rcoverridech8 = pickchannel(8, JoyChannels[8].axis, JoyChannels[8].reverse,
                            JoyChannels[8].expo);
                    ...
                    //Console.WriteLine("{0} {1} {2} {3}", MainUI.comPort.MAV.cs.rcoverridech1, MainUI.comPort.MAV.cs.rcoverridech2, MainUI.comPort.MAV.cs.rcoverridech3, MainUI.comPort.MAV.cs.rcoverridech4);
                }
                catch (SharpDX.SharpDXException ex)
                {
                    log.Error(ex);
                    clearRCOverride();
                    MainUI.MainUIInstance.Invoke((System.Action)
        ...
        public void clearRCOverride()
        {
            ...
            rc.target_component = MainUI.comPort.MAV.compid;
            rc.target_system = MainUI.comPort.MAV.sysid;
            ...
            try
            {
                MainUI.comPort.sendPacket(rc, rc.target_system, rc.target_component);
            ...
            if (but.buttonno != -1)
            {
                ...
                switch (but.function)
                {
                    case buttonfunction.ChangeMode:
                        ...
                        if (mode != null)
                        {
                            MainUI.MainUIInstance.BeginInvoke((System.Windows.Forms.MethodInvoker) delegate()
                            {
                                try
                                {
                                    MainUI.comPort.setMode(mode);
                                }
                                catch
                                {
                    ...
                    case buttonfunction.Mount_Mode:
                        ...
                        MainUI.MainUIInstance.BeginInvoke((System.Windows.Forms.MethodInvoker) delegate()
                        {
                            try
                            {
                                MainUI.comPort.setParam("MNT_MODE", but.p1);
                            }
                            catch
                            {
                    ...
                    case buttonfunction.TakeOff:
                        MainUI.MainUIInstance.BeginInvoke((System.Windows.Forms.MethodInvoker) delegate()
                        {
                            try
                            {
                                MainUI.comPort.setMode("Guided");
                                if (MainUI.comPort.MAV.cs.firmware == Firmwares.ArduCopter2)
                                {
                                    MainUI.comPort.doCommand(MAVLink.MAV_CMD.TAKEOFF, 0, 0, 0, 0, 0, 0, 2);
                                }
                                else
                                {
                                    MainUI.comPort.doCommand(MAVLink.MAV_CMD.TAKEOFF, 0, 0, 0, 0, 0, 0, 20);
                                }
                            }
                            catch
                            {
                    ...
                    case buttonfunction.Do_Set_Relay:
                        MainUI.MainUIInstance.BeginInvoke((System.Windows.Forms.MethodInvoker) delegate()
                        {
                            try
                            {
                                ...
                                MainUI.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_RELAY, number, state, 0, 0, 0, 0, 0);
                            }
                            catch
                            {
                    ...
                    case buttonfunction.Digicam_Control:
                        MainUI.comPort.setDigicamControl(true);
                        break;
                    case buttonfunction.Do_Repeat_Relay:
                        MainUI.MainUIInstance.BeginInvoke((System.Windows.Forms.MethodInvoker) delegate()
                        {
                            try
                            {
                                ...
                                int time = (int) but.p3;
                                MainUI.comPort.doCommand(MAVLink.MAV_CMD.DO_REPEAT_RELAY, relaynumber, repeat, time, 0,
                                    0, 0, 0);
                            }
                            catch
                            {
                    ...
                    case buttonfunction.Do_Set_Servo:
                        MainUI.MainUIInstance.BeginInvoke((System.Windows.Forms.MethodInvoker) delegate()
                        {
                            try
                            {
                                ...
                                int pwm = (int) but.p2;
                                MainUI.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, channel, pwm, 0, 0, 0, 0, 0);
                            }
                            catch
                            {
                    ...
                    case buttonfunction.Do_Repeat_Servo:
                        MainUI.MainUIInstance.BeginInvoke((System.Windows.Forms.MethodInvoker) delegate()
                        {
                            try
                            {
                                ...
                                MainUI.comPort.doCommand(MAVLink.MAV_CMD.DO_REPEAT_SERVO, channelno, pwmvalue,
                    ...
                    case buttonfunction.Toggle_Pan_Stab:
                        MainUI.MainUIInstance.BeginInvoke((System.Windows.Forms.MethodInvoker) delegate()
                        {
                            try
                            {
                                float current = (float) MainUI.comPort.MAV.param["MNT_STAB_PAN"];
                                float newvalue = (current > 0) ? 0 : 1;
                                MainUI.comPort.setParam("MNT_STAB_PAN", newvalue);
                            }
                            catch
                            {
                    ...
                    case buttonfunction.Gimbal_pnt_track:
                        MainUI.MainUIInstance.BeginInvoke((System.Windows.Forms.MethodInvoker) delegate()
                        {
                            try
                            {
                                MainUI.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_ROI, 0, 0, 0, 0,
                                    MainUI.comPort.MAV.cs.gimballat, MainUI.comPort.MAV.cs.gimballng,
                                    (float) MainUI.comPort.MAV.cs.GimbalPoint.Alt);
                            }
                            catch
                            {
                    ...
                    case buttonfunction.Mount_Control_0:
                        MainUI.MainUIInstance.BeginInvoke((System.Windows.Forms.MethodInvoker) delegate()
                        {
                            try
                            {
                                MainUI.comPort.setMountControl(0, 0, 0, false);
                            }
                            catch
                            {
        ...
        */
        ...
        short pickchannel(int chan, joystickaxis axis, bool rev, int expo)
        {
            ...
            if (MainUI.comPort.MAV.param.Count > 0)
            {
                try
                {
                    if (MainUI.comPort.MAV.param.ContainsKey("RC" + chan + "_MIN"))
                    {
                        min = (int) (float) (MainUI.comPort.MAV.param["RC" + chan + "_MIN"]);
                        max = (int) (float) (MainUI.comPort.MAV.param["RC" + chan + "_MAX"]);
                        trim = (int) (float) (MainUI.comPort.MAV.param["RC" + chan + "_TRIM"]);
                    }
                    else
                    {
                        min = 1000;
