...
namespace SKYROVER.GCS.DeskTop.Controls
{
    ...
    { "calibration",        Calibration::create },
        ...
        {
            homemarker.Position = MainUI.comPort.MAV.cs.HomeLocation;

            myGMAP1.Position = homemarker.Position;

            myGMAP1.MapProvider =MainUI.MainUIInstance.getGMAP.MapProvider;
            myGMAP1.MaxZoom = 22;
            myGMAP1.Zoom = 16;
           ...
           // Utilities.ThemeManager.ApplyThemeTo(this);
             ...
             Utilities.Tracking.AddPage(this.GetType().ToString(), this.Text);
        }
        ...
        private void StartSITL(string exepath, string model, string homelocation, string extraargs = "", int speedup = 1)
        {
            ...
            //MainUI.View.ShowScreen(MainUI.View.screens[0].Name);

            var client = new MissionPlanner.Comms.TcpSerial();

            try
            {
                client.client = new TcpClient("127.0.0.1", 5760);

                MainUI.comPort.BaseStream = client;

                SITLSEND = new UdpClient("127.0.0.1", 5501);

                Thread.Sleep(200);

                MainUI.MainUIInstance.doConnect(MainUI.comPort, "preset", "5760");
            }
            catch
            {
        ...
        static internal void rcinput()
        {
            try
            {
                ...
                Array.ConstrainedCopy(BitConverter.GetBytes((ushort) MainUI.comPort.MAV.cs.rcoverridech1), 0, rcreceiver,0, 2);
                Array.ConstrainedCopy(BitConverter.GetBytes((ushort) MainUI.comPort.MAV.cs.rcoverridech2), 0, rcreceiver,2, 2);
                Array.ConstrainedCopy(BitConverter.GetBytes((ushort) MainUI.comPort.MAV.cs.rcoverridech3), 0, rcreceiver,4, 2);
                Array.ConstrainedCopy(BitConverter.GetBytes((ushort) MainUI.comPort.MAV.cs.rcoverridech4), 0, rcreceiver,6, 2);
                Array.ConstrainedCopy(BitConverter.GetBytes((ushort) MainUI.comPort.MAV.cs.rcoverridech5), 0, rcreceiver,8, 2);
                Array.ConstrainedCopy(BitConverter.GetBytes((ushort) MainUI.comPort.MAV.cs.rcoverridech6), 0, rcreceiver,10, 2);
                Array.ConstrainedCopy(BitConverter.GetBytes((ushort) MainUI.comPort.MAV.cs.rcoverridech7), 0, rcreceiver,12, 2);
                Array.ConstrainedCopy(BitConverter.GetBytes((ushort) MainUI.comPort.MAV.cs.rcoverridech8), 0, rcreceiver,14, 2);
