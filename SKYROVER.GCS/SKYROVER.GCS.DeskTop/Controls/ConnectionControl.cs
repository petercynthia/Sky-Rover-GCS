...
namespace SKYROVER.GCS.DeskTop.Controls
{
    public partial class ConnectionControl : UserControl
    {
        ...
        public void UpdateSysIDS()
        {
            ...
            foreach (var port in MainUI.Comports.ToArray())
            {
                ...
                foreach (int item in list)
                {
                    ...
                    if(temp.port == MainUI.comPort && temp.sysid == MainUI.comPort.sysidcurrent && temp.compid == MainUI.comPort.compidcurrent)
                    {
                        selectidx = idx;
                    }
                }
        ...
        private void CMB_sysid_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            foreach (var port in MainUI.Comports)
            {
                if (port == temp.port)
                {
                    MainUI.comPort = port;
                    MainUI.comPort.sysidcurrent = temp.sysid;
                    MainUI.comPort.compidcurrent = temp.compid;

                    if (MainUI.comPort.MAV.param.Count == 0 && !(Control.ModifierKeys == Keys.Control))
