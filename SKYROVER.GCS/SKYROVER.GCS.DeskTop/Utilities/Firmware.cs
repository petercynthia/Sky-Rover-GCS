...
namespace SKYROVER.GCS.DeskTop.Utilities
{
    public class Firmware
    {
        ...
        public event MissionPlanner.ProgressEventHandler Progress;
        ...
        public bool update(string comport, software temp, string historyhash)
        {
            ...
            Utilities.Tracking.AddFW(temp.name, board.ToString());
        ...
        private void AttemptRebootToBootloader()
        {
            ...
            if (MainUI.comPort.BaseStream is SerialPort)
            {
                try
                {
                    updateProgress(-1, "Look for HeartBeat");
                    // check if we are seeing heartbeats
                    MainUI.comPort.BaseStream.Open();
                    MainUI.comPort.giveComport = true;

                    if (MainUI.comPort.getHeartBeat().Length > 0)
                    {
                        updateProgress(-1, "Reboot to Bootloader");
                        MainUI.comPort.doReboot(true, false);
                        MainUI.comPort.Close();
                    }
                    else
                    {
                        updateProgress(-1, "No HeartBeat found");
                        MainUI.comPort.BaseStream.Close();
