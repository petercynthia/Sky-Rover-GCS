      ...
      static System.Reflection.Assembly CurrentDomain_AssemblyResolve(object sender, ResolveEventArgs args)
      {
         if(args.Name.StartsWith("System.Data.SQLite", StringComparison.OrdinalIgnoreCase))
         {
            ...
            if(!File.Exists(dll))
            {
               ...
               {
                  using(MemoryStream gzipDll = new MemoryStream((IntPtr.Size == 8 ? Core.Properties.Resources.System_Data_SQLite_x64_NET2_dll : Core.Properties.Resources.System_Data_SQLite_x86_NET2_dll)))
                  {
               ...
               {
                  using(MemoryStream gzipDll = new MemoryStream((IntPtr.Size == 8 ? Core.Properties.Resources.System_Data_SQLite_x64_NET4_dll : Core.Properties.Resources.System_Data_SQLite_x86_NET4_dll)))
                  {
                     using(var gs = new System.IO.Compression.GZipStream(gzipDll, System.IO.Compression.CompressionMode.Decompress))
                     {
                        using(MemoryStream exctDll = new MemoryStream())
                  ...
                  using(DbTransaction tr = cn.BeginTransaction())
                  {
                     try
                     {
                        using(DbCommand cmd = cn.CreateCommand())
                        {
                           cmd.Transaction = tr;
#if !PocketPC
                           cmd.CommandText = Core.Properties.Resources.CreateTileDb;
#else
